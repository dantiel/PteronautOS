...
void AsyncWebServerRequest::send(AsyncWebServerResponse *response) {
  ...
  // OOM-safe: beginResponse() now uses new(std::nothrow) and can return NULL
  // when the heap is exhausted. Drop the request silently (the client's
  // connection times out and the browser retries) instead of dereferencing
  // a null response — a null deref is a hard Exception on the ESP8266.
  if (response == NULL) {
    return;
  }
