...
AsyncWebServer::AsyncWebServer(uint16_t port) : _server(port) {
  _catchAllHandler = new (std::nothrow) AsyncCallbackWebHandler();
  _server.onClient(
...
AsyncCallbackWebHandler &AsyncWebServer::on(
  ...
  AsyncCallbackWebHandler *handler = new (std::nothrow) AsyncCallbackWebHandler();
  handler->setUri(uri);
...
AsyncStaticWebHandler &AsyncWebServer::serveStatic(const char *uri, fs::FS &fs, const char *path, const char *cache_control) {
  AsyncStaticWebHandler *handler = new (std::nothrow) AsyncStaticWebHandler(uri, fs, path, cache_control);
  addHandler(handler);
  return *handler;
}
